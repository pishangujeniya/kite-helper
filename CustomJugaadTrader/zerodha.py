...
"""
LAST UPDATED ON 15th January 2022 from
...
class Zerodha(KiteConnect):
    ...
    def load_session(self, path=None):
        ...
        if path == None:
            path = os.path.join(IniConfiguration.get_value("system", "data_dir_path"),
                                IniConfiguration.get_value("kite", "session_file"))
        try:
    ...
    def load_creds(self, path=None):
        if path == None:
            path = os.path.join(IniConfiguration.get_value("system", "data_dir_path"),
                                IniConfiguration.get_value("kite", "creds_file"))
